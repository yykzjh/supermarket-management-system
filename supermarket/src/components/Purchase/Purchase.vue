      ...
      <el-table :data="buyList" border stripe>
        ...
        <el-table-column prop="if_finish" label="完成状态">
          <template slot-scope="scope">
            ...
            <el-switch v-model="scope.row.if_finished" v-if="!scope.row.if_finish" @change="changeState(scope.row.good_id, scope.row.supplier_id)"></el-switch>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async changeState(goodid, supplierid){ // 接口有错
