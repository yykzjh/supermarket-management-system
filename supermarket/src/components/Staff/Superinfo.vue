      ...
      <el-table id="example" :data="userList.slice((pagenum-1)*pagesize, pagenum*pagesize)" border stripe>
        ...
        <el-table-column label="角色" prop="role_id">
          <template slot-scope="scope">
            <p v-if="scope.row.role_id == 3">超级管理员</p>
    ...
    <el-dialog
      ...
      <el-form :model="addUserForm" :rules="editFormRules" ref="addUserFormRef" label-width="90px">
        ...
        <el-form-item label="权限">
          ...
          <el-radio v-model="addUserForm.role_id" label=3>超级管理员</el-radio>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods: {
      ...
      async getUserList() {
        ...
        const {data: res3} = await this.$http.get('Users/AllUsers', {
          params: { name: 'superadmin'}
        })
        this.userList = res1.users.concat(res2.users).concat(res3.users)
      ...
      async showAllotRole(info) {
        console.log(info)
        this.changeRoleNum = parseInt(info.role_id)
        this.changeRoleId = info.id
        this.changeRoleSee = true
      },
      async AllotRole(){
        const {data: res} = await this.$http.post('/Users/NewRoleId', {
          username: this.changeRoleId, role_id: this.changeRoleNum
        })
        if(res.StatusCode !== 200)
